...
function MM_Init() {
    ...
    addStyle('.MTFlexGridTitleRow { font-weight: 600; height: 56px; position: sticky; top: 0; ' + panelBackground + '}');
    ...
    addStyle('.MTFlexGridDCell {white-space: nowrap;}');
    addStyle('.MThRefClass2 {font-family: MonarchIcons, "Oracle", sans-serif;}');
    addStyle('.MTFlexGridDCell2 { text-align: right; }');
    addStyle('.MTFlexGridSCell,.MTFlexGridS3Cell, .MTFlexGridSCell2 {  background-color: ' + detailLine + ';font-size: 15px; height: 34px;' + standardText + ' font-weight: 600; }');
    ...
    addStyle('.MTFlexCellArrow, .MTTrendCellArrow, .MTTrendCellArrow2 {' + panelBackground + standardText + 'width: 26px; height: 26px; font-size: 17px; font-family: MonarchIcons, sans-serif; transition: 0.1s ease-out; cursor: pointer; border-radius: 100%; border-style: none;}');
    ...
    addStyle('.MTSideDrawerSummaryData {overflow: hidden; white-space: nowrap; text-overflow: ellipsis;}');
...
function MF_QueueAddTitle(inCol,inTitle,p,inIsHidden) {
    ...
    MTFlexTitle.push({"Col": inCol, "Title": inTitle,"isSortable": p.isSortable, "Width": p.Width, "Format": p.Format, "FormatExtended": [], "ShowPercent": p.ShowPercent, "ShowPercentShade": p.ShowPercentShade, "ShowSort": p.ShowSort, "isHidden": useHide, "Indicator": p.Indicator});
...
function MF_GridDraw(inRedraw) {
    if(debug == 1) console.log(MTFlex,MTFlexTitle,MTFlexRow);
...
function MT_GridDrawDetails() {
    ...
    function MT_GridDrawRow(isSubTotal) {
        ...
        if(useDesc.startsWith('')) useDesc = useDesc.slice(2);
        useStyle = useStyle + '2';
        for (let j = 1; j < MTFlexTitle.length; j++) {
           const thisTitle = MTFlexTitle[j];
            if(thisTitle.isHidden != true) {
                useValue = useRow[j + MTFields];useValue2 = useValue;
                if(useValue != null) {
                    // -1 Date, 1=Decimals, 2=No Decimals
                    switch(thisTitle.Format) {
                        ...
                        case 3:
                        case 13:
                            useValue2 = useValue.toLocaleString('en-US');break;
                        default:
                            useValue2 = useValue;
                    }
                }
                if(thisTitle.Format < 1) {
                    ...
                    if((thisTitle.ShowPercent != null)) {
                        switch (thisTitle.ShowPercent.Type) {
                            case 'Row':
                                workValue = 0;workValue2 = 0;
                                for (let k = 0; k < thisTitle.ShowPercent.Col1.length; k++) {
                                    rowVal = useRow[MTFields + thisTitle.ShowPercent.Col1[k]];
                                    if(k == 0) {workValue = rowVal} else {workValue = workValue - rowVal;}
                                }
                                for (let k = 0; k < thisTitle.ShowPercent.Col2.length; k++) {
                                    rowVal = useRow[MTFields + thisTitle.ShowPercent.Col2[k]];
                                     if(k == 0) {workValue2 = rowVal} else {workValue2 = workValue2 - rowVal;}
                                }
                                pct = MT_GridPercent(workValue,workValue2,thisTitle.ShowPercentShade,1,useRow.IgnoreShade);
                                break;
                            case 'Column':
                                rowVal = useRow.BasedOn -1;
                                rowVal = Subtotals[rowVal];
                                if(MTFlexRow[rowVal] != undefined) {
                                    workValue = MTFlexRow[rowVal][j + MTFields];
                                    pct = MT_GridPercent(workValue,useValue,thisTitle.ShowPercentShade,2,useRow.IgnoreShade);
                                }
                                break;
                            default:
                                console.log('MT-Tweaks: Invalid ShowPercent.Type: ' + thisTitle.ShowPercent.Type); return;
                        }
                        useValue2 = useValue2 + ' ' + pct[0]; useStyle2 = pct[1];
                    }
                    ...
                    if(MTFlex.AutoCard != undefined) {
                        if(MTFlex.AutoCardColumn == j) {
                            runCard = false;
                            if(MTFlex.AutoCardSection == -1 && useRow.Section % 2 == 1) runCard = true;
                            if(MTFlex.AutoCardSection > 0) {
                                if((isSubTotal == true && MTFlex.AutoCardSection == useRow.Section) || (isSubTotal == false && MTFlex.AutoCardSection -1 == useRow.Section)) {runCard = true;}
                            }
                            if(runCard == true) {
                                MTP.Col = 1 + (MTFlexCard.length+1);
                                if(isSubTotal == true || MTFlex.AutoCardSection == -1) {MTP.Subtitle = useDesc;} else {MTP.Subtitle = MTFlex.AutoCard;}
                                MTP.Title = useValue2;
                                MTP.Style = css_green;
                                MF_QueueAddCard(MTP);
                            }
...
function MT_GridDrawEmbed(inSection,inCol,inValue, inDesc) {
    switch (MTFlex.Name) {
        ...
        case 'MTAccounts':
            ...
            break;
        case 'MTInvestments':
            if (inCol == 9) { if(inValue == 0) return ''; return inValue < 0 ? css_red : css_green ;}
            break;
    }
    return '';
}
...
function MF_GridRollup(inSection,inRoll,inBasedOn,inName) {
    ...
    for (let i = 0; i < MTFlexRow.length; i++) {
         if(MTFlexRow[i].Section == inRoll) {
             ...
             for (let j = 1; j < MTFlexTitle.length; j++) { if(MTFlexTitle[j].Format == 1 || MTFlexTitle[j].Format == 2) {Subtotals[j] += MTFlexRow[i][MTFields + j];} }
    }}
    MTP = [];
    ...
    for (let j = 1; j < MTFlexTitle.length; j++) {
        if(MTFlexTitle[j].Format == 1 || MTFlexTitle[j].Format == 2) {MTFlexRow[MTFlexCR][MTFields+j] = Subtotals[j];} else {MTFlexRow[MTFlexCR][MTFields+j] = '';}
    }
}
...
function MF_GridGroupByPK() {
    ...
    let i = 2
    for (i = 2; i < useSec; i+=2) { MF_GridRollup(i-1,i,i);}
    MF_GridRollup(i-1,i,i);
}
...
async function MenuReportsAccountsGo() {
    ...
    async function MenuReportsAccountsGoStd(){
                ...
                if(snapshotData.accounts[i].hideFromList == false || skipHidden == 0) {
                    if(snapshotData.accounts[i].includeInNetWorth == true || skipHidden2 == 0) {
                        ...
                        if(useBalance !=0 || getAccountUsed(MTP.UID) == true || pastBalance != 0) {
                            ...
                            MTFlexRow[MTFlexCR][MTFields+12] = useBalance + MTFlexRow[MTFlexCR][MTFields+12];
                            if(MTFlex.Button2 != 1) {
        ...
        if(MTFlex.Button2 == 2) {
            ...
            MTFlex.AutoCard = 'Net Change'; MTFlex.AutoCardSection = 2; MTFlex.AutoCardColumn = 10;
        } else {
            cards=0;
            for (let i = 0; i < 5; i += 1) {
...
async function MenuReportsInvestmentsGo() {
    ...
    MTFlex.CanvasTitle = 'height: 20px;font-size: 13.5px;';
    MTFlex.CanvasData = 'height: 20px;';
    MTFlex.TriggerEvent = 0;
    MTFlex.TriggerEvents = false;
    MF_GridOptions(2,['No Grouping','Group by Institution','Group by Account','Group by Account Subtype','Group by Stock Type']);
    MF_GridOptions(4,getAccountGroupInfo());
    MTFlex.SortSeq = ['1','2','3','4'];
    MTFlex.Title1 = 'Investments Report';

    MTP = [];MTP.isSortable = 1; MTP.Format = 0;
    MTP.Width = '220px;';MF_QueueAddTitle(0,'Name',MTP);
    MF_QueueAddTitle(1,'Institution',MTP,MTFlex.Button2);
    MF_QueueAddTitle(2,'Account',MTP,MTFlex.Button2);
    MF_QueueAddTitle(3,'Subtype',MTP,MTFlex.Button2);
    MF_QueueAddTitle(4,'Type',MTP,MTFlex.Button2);
    MTP.isSortable = 2;
    MTP.Format = 11;MTP.Width = '5%';MF_QueueAddTitle(5,'Price',MTP);
    MTP.Format = 13;MTP.Width = '5%';MF_QueueAddTitle(6,'Qty',MTP);
    MTP.Format = 1;MTP.Width = '8%';MF_QueueAddTitle(7,'Value',MTP);
    MTP.Format = 1;MTP.Width = '8%';MF_QueueAddTitle(8,'Cost Basis',MTP);
    MTP.Format = 1;MTP.Width = '8%';MF_QueueAddTitle(9,'Gain/Loss $',MTP);
    MTP.Format = 1;MTP.Width = '8%';MF_QueueAddTitle(10,'Gain/Loss %',MTP);
    MTP.Format = 1;MTP.Width = '8%';MF_QueueAddTitle(11,'% of Acct',MTP);

    let lowerDate = formatQueryDate(MTFlexDate1);
    let higherDate = formatQueryDate(MTFlexDate2);
    let useSubType = '',useCostBasis=0, useTitle = '', sumPortfolio = 0, cashValue = 0, sumCash = 0, accQueue = [];
    ...
    if(MTFlex.Button2 > 0) {MF_GridGroupByPK();}
    MTSpawnProcess = 1;

    async function BuildInvestmentCards() {

        MF_QueueAddCard({Col: 0, Title: getDollarValue(sumPortfolio,false), Subtitle: 'Total Brokerage', Style: css_green});
        MTFlex.AutoCard = ''; MTFlex.AutoCardSection = -1; MTFlex.AutoCardColumn = 7;
    ...
    async function BuildInvestmentCash() {
        ...
        for (const acc of accQueue) {
            ...
            if(cashValue != 0) {
                ...
                MTFlexRow[MTFlexCR][MTFields+5] = null;
                MTFlexRow[MTFlexCR][MTFields+6] = null;
                MTFlexRow[MTFlexCR][MTFields+7] = cashValue;
                MTFlexRow[MTFlexCR][MTFields+8] = null;
    ...
    async function BuildInvestmentHoldings() {
        ...
        for (const edge of snapshotData.portfolio.aggregateHoldings.edges) {
            ...
            for (const holding of holdings) {
                ...
                if (useTitle.length > 46) {useTitle = useTitle.slice(0, 46) + ' ...'};
                MTFlexRow[MTFlexCR][MTFields] = useTitle;
...
async function MenuReportsTrendsGo() {
    ...
    if(MTFlex.Button2 > 2) {
        ...
        lowerDate = lowerDate.setDate(lowerDate.getDate() + 1);
        ...
        if(retGroup.TYPE == 'transfer') {
            ...
            if(retGroup.TYPE == 'expense') {
                if(retGroup.ISFIXED == true) {
                    MTFlexRow[i].BasedOn = 2;MTFlexRow[i].Section = 4;
                } else {
                    MTFlexRow[i].BasedOn = 3;MTFlexRow[i].Section = 6;
                }
    ...
    if(typeof cn === 'string') {
        ...
        switch (cn) {
            ...
            case 'MTSideDrawerSummaryTag':
                onClickExpandHistory(event.target);
                break;
...
async function onClickExpandHistory(useTarget) {
    ...
    const data = useTarget.getAttribute('data');
    const dataYear = data.split('|')[0];
    const dataMonth = data.split('|')[1];
...
function getDates(InValue,InDate) {
    ...
    switch (InValue) {
        ...
        case 'd_LastYear':d.setFullYear(d.getFullYear() - 1);return d;
